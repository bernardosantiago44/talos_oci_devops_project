...
// ─── Mappers ─────────────────────────────────────────────────────
function mapBackendRowToDetailDto(row) {
    return {
        id: row.workItemId,
...
function mapToListItemDto(detail) {
    return {
        id: detail.id,
        ...
        assignees: detail.assignees.map((a) => a.user),
        tags: detail.tags,
    };
}
...
export const workItemService = {
    ...
    async createWorkItem(input) {
        ...
        const result = await apiClient.post('/workitems', body);
        if (!result.success) {
            return { success: false, data: null, message: result.message };
        }
    ...
    async updateWorkItem(id, input) {
        ...
        if (input.status) {
            const statusResult = await apiClient.put(`/workitems/${id}/status`, { status: toBackendStatus(input.status) });
